import { useRemoveSessionMutation } from "../../services/sessionsApi";
import UpdateSession from "../UpdateSession/UpdateSession";
import { Movie, Session } from "../../types/authTypes";

interface SessionCardProps {
  movieId: Movie;
  reloadMovies: () => void;
}

const openUpdateModal = (sessionId: string) => {
  const modal = document.getElementById(
    `add_session_${sessionId}`
  ) as HTMLDialogElement | null;
  modal?.showModal();
};

export default function SessionCard({ movieId }: SessionCardProps) {
  const [removeSession] = useRemoveSessionMutation();

  const handleDeleteSession = (sessionId: string) => {
    removeSession({ movieId: movieId.movieId, sessionId });
  };

  return (
    <div className="flex flex-col gap-5 mt-5">
      <h1 className="text-2xl font-bold mt-10 text-black dark:text-white">
        {movieId.tmdbDetails.title}
      </h1>
      <div className="flex flex-wrap justify-start gap-5">
        {movieId.sessions.map((session: Session) => (
          <div
            key={session._id}
            className="card bg-base-100 image-full w-96 shadow-sm"
          >
            <figure>
              <img
                src={`https://image.tmdb.org/t/p/w500${movieId.tmdbDetails.poster_path}`}
                alt={movieId.tmdbDetails.title ?? "Movie Poster"}
              />
            </figure>
            <div className="card-body flex justify-between">
              <div className="flex flex-col gap-4">
                <h2 className="card-title text-yellow-400">
                  {movieId.tmdbDetails.title ?? "No Title"}
                </h2>
                <div>
                  <p>{new Date(session.dateTime).toLocaleDateString()}</p>
                  <p>{new Date(session.dateTime).toLocaleTimeString()}</p>
                </div>
                <h2 className="card-title text-yellow-400">Seats:</h2>
                <div className="flex flex-wrap gap-2">
                  {session.seats.map((seat) => (
                    <div
                      key={seat.seatNumber}
                      className={`p-2 rounded-lg ${
                        seat.isBooked ? "bg-green-700" : "bg-red-700"
                      }`}
                    >
                      <span>{seat.seatNumber}</span>
                    </div>
                  ))}
                </div>
              </div>

              <div className="card-actions justify-end">
                <button
                  className="btn bg-red-600 border-none"
                  onClick={() => session._id && handleDeleteSession(session._id)}
                  aria-label="Delete session"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="white" className="size-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                  </svg>
                </button>

                <button
                  className="btn bg-blue-500 border-none"
                  onClick={() => session._id && openUpdateModal(session._id)}
                  aria-label="Edit session"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="white" className="size-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                  </svg>
                </button>
              </div>
            </div>

            {session._id && (
              <UpdateSession
                movieId={movieId.movieId}
                _id={session._id}
                dateTime={session.dateTime}
                ticketPrice={session.price}
                countOfSeats={session.seats}
              />
            )}
          </div>
        ))}
      </div>
    </div>
  );
}