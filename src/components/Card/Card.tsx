...
export default function Card({ movie }: CardProps) {
  const { tmdbDetails, movieId } = movie;

  const handleOpenModal = () => {
    const dialog = document.getElementById(
      `add_session_${movieId}`
      ...
      <div className="card-body flex justify-between">
        ...
        <div className="card-actions justify-between items-center flex-row">
          <div className="flex flex-row items-center gap-2">
            ...
            <h1 className="font-extrabold text-xl text-yellow-300">
              {tmdbDetails.vote_average.toFixed(1)}
            </h1>
          </div>
          <button
            onClick={handleOpenModal}
            className="btn bg-yellow-400 dark:text-yellow-400 dark:bg-gray-900"
          >
            Add new session
          </button>
        </div>

        <AddSession movieId={movieId} />
      </div>
    </div>
  );
}