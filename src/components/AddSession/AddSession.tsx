...
import { useAddSessionMutation } from "../../services/sessionsApi";

interface AddSessionProps {
  movieId: number;
}

export default function AddSession({ movieId }: AddSessionProps) {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      countOfSeats: Yup.number()
        .min(1, "Seats must be greater than 0")
        .required("Count of seats is required"),
    }),
    onSubmit: async (values, { resetForm }) => {
      const seats = Array.from({ length: values.countOfSeats }, (_, i) => ({
        seatNumber: i + 1,
        isBooked: false,
      }));
  ...
  return (
    <dialog
      id={`add_session_${movieId}`}
      className="modal modal-bottom sm:modal-middle"
    >
      <div className="modal-box">
        <h1 className="text-black dark:text-white font-bold text-2xl text-start pb-4">
          Add new Session:
        </h1>
